...
class BLEDataProcessor {
  ...
  Future<void> _sendInChunks(List<int> bytes, int mtu) async {
    for (var offset = 0; offset < bytes.length; offset += mtu) {
      ...
      await controller!.writeCharacteristic!.write(
        chunk,
  ...
  void listenToNotifications() {
    ...
    _packetQueue.clear();
    controller!.notifyCharacteristic!.onValueReceived.listen(
      (value) async {
        try {
  ...
  void _handleTimeoutFailure() {
    controller!.notifyStatus(
      'Failed to receive data after $_maxRetries retries',
    );
